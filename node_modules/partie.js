...
if (fmpConstants == null) {
	console.log('WARNING !!! require fmpConstants failed');
} else {
	console.log(JSON.stringify(fmpConstants));
}
var randomCurrentMaree = Math.floor((Math.random() * 3));
var randomNextMaree = Math.floor((Math.random() * 3));
console.log('fmpConstants.MAREES[' + randomCurrentMaree + ']:' + fmpConstants.MAREES[randomCurrentMaree]);
var datas = {
	tour: 0,
	tourPlayer: 0,
	tourPoints: 15,
	...
	pieces: [
		...
		// new Piece(2, 1, fmpConstants.PIECE_TYPE.TANK, 2, 8), 
...
this.getPlateau = function() {
	...
	plateau.forEach(function(ligne) {
		...
		ligne.forEach(function (aCase) {
			this.push(new fmpCase.FMPCase(aCase.caseType, aCase.x, aCase.y));
		}, ligneCopy);
		this.push(ligneCopy);
	}, plateauCopy);
	return plateauCopy;
...
this.init = function() {
	...
	datas.currentMaree = fmpConstants.MAREES[randomMaree];
